$(function () {
    ...
    $('#location').on('input', function () {
        $('#area_id').remove();
        $('#search-by-location').prop('action', BASE_URL + 'uk');
        form_changed = true;
    });
    ...
    if ($('#latbox').val() != 0) {
        ...
        if (typeof laracasts !== 'undefined') {
            if (typeof laracasts.latitude !== 'undefined') {
                latitude = laracasts.latitude;
            }

            if (typeof laracasts.latitude !== 'undefined') {
                longitude = laracasts.longitude;
            }
        }
...
function DiscoverMapWidgetInit() {
    ...
    infoBubble();
    var everciseGroups = JSON.parse(EVERCISE_GROUPS);
    var evercisePolygon = JSON.parse(EVERCISE_POLYGON);
    ...
    if (!everciseGroups.length) {
        ...
        for (i = 0; i < everciseGroups.length; i++) {
            var evercisegroup = everciseGroups[i];
            ...
            if (venue) {
                ...
                var content = '<div class="info-window recommended-block">' +
                    '<div class="block-header">' +
                    '<a href="/evercisegroups/' + evercisegroup.id + '">' + evercisegroup.name + '</a>' +
                    '</div>' +
                    ...
                    '</div>' +
                    '<div class="recommended-aside">' +
                    '<img class="date-icon" src="/img/date_icon.png">' +
                    '<span>' + moment(sessions[0].date_time).format('DD MMM YYYY - hh:mma') + '</span>' +
                    '</div>' +
                    '</div>' +
                    '<div class="block-footer">' +
                    '<span>price: &pound;' + evercisegroup.default_price + '<span>' +
                    '</div>' +
                    '</div>';
        ...
        if(evercisePolygon.length > 0) {
            var polygonCoords = [];
            for (i = 0; i < evercisePolygon.length; i++) {
                // Define the LatLng coordinates for the polygon's path.
                polygonCoords.push(new google.maps.LatLng(evercisePolygon[i][0], evercisePolygon[i][1]));
            }
