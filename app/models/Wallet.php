...
class Wallet extends \Eloquent
{
    ...
    public function deposit($amount, $description, $type, $sessionmember_id = 0, $token = 0, $transactionId = 0, $paymentMethod = 0, $payer_id = 0)
    {
        $this->transaction($amount, $description, $type, $sessionmember_id, $token, $transactionId, $paymentMethod, $payer_id );
    }

    public function withdraw($amount, $description, $type, $sessionmember_id = 0, $token = 0, $transactionId = 0, $paymentMethod = 0, $payer_id = 0)
    {
        $this->transaction(-$amount, $description, $type, $sessionmember_id, $token, $transactionId, $paymentMethod, $payer_id );
    }
    ...
    protected function transaction($amount, $description, $type, $sessionmember_id = 0, $token = 0, $transactionId = 0, $paymentMethod = 0, $payer_id = 0)
    {
        ...
        Log::info('WALLET\TRANSACTION::: user:' . $user_id . ' type:'.$type);

        if ($type == 'referral')
            event('user.referral.completed', [$this->user, $transaction, $newBalance]);
        else if ($type == 'referral_signup')
