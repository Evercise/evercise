...
class EvercisegroupsController extends \BaseController
{
    ...
    public function parseUrl($all_segments = '')
    {
        ...
        if ($link) {
            ...
            switch ($link->type) {
                case 'AREA':
                case 'STATION':
                    $this->getSearch($link->getArea());
                    break;
                case 'CLASS':
                    $this->show($link->getArea()->id);
                    break;
            }
        }
        ...
        switch ($parsed['type']) {
            case 'class':
                return $this->getClass($parsed);
                break;
            case 'instructor':
                return $this->getInstructor($parsed);
                break;
            case 'page':
                return $this->getPage($parsed);
                break;
            default:
                return $this->getSearch($parsed);
    ...
    /uk
    /uk/london
    /uk/london/areas/{area.permalink}
    /uk/london/stations/{station.permalink}
    /uk/london?page=2
    /uk/{location.permalink}
    /uk/{location.permalink}/{type.permalink}
    /uk/classes/{class.permalink_hash}
    /instructors/{user.permalink}
    /uk/{localed_page.permalink}
    /uk/{page.permalink}
            */
    /*
