				...
				<tr>
					<td valign="top">
						...
						<table width="600" height="135" cellpadding="0" cellspacing="0" bgcolor="#ffffff" style="color:#000000; padding: 0px 0px 0px 0px">
							<tr>
								<td>
									<a href="http://www.evercise.com" target="_blank" style="color: #336699;font-weight: normal;text-decoration: underline;">{{HTML::image('img/top_banner.jpg', 'Everybody exercise', array('width' => 600, 'id' => 'banner', 'height' => 135));}}</a>
								</td>
							</tr>
						</table>
						...
						<table width="600px" height="73px" cellpadding="0" cellspacing="0" bgcolor="#ffffff" style=" font-size:18px; line-height:21.6px; color:#000000; padding: 0px 0px 0px 0px; font-family: Exo, lato, Helvetica, ‘Helvetica Neue’, Arial; ">
							<tr align="center">
								<td valign="top">
									<span style="font-size:24px; line-height:28.8px; font-weight:bold">Hi {{{ $name }}}, You've got a message from {{{ $user_name }}} regarding your {{{ $group }}} session at {{{ date('h:m', strtotime($dateTime)) }}} on {{{ date('d-M-Y', strtotime($dateTime)) }}}</span><br>
									<span>{{{ $subject }}}</span>
								</td>
							</tr>
						</table>
