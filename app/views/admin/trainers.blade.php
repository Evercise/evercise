...
<div class="row">
    <ul id="user_list">
       @foreach($users as $key => $theUser)
            <li>
                <h3 class="ul_userName"><span class="ul_firstName">{{ $theUser->id .' : '.$theUser->first_name }}</span> <span class="ul_lastName">{{ $theUser->last_name }} </span> ({{ $theUser->display_name }})</h3>

                <p>
                    <small class="text-muted">Phone:</small> <span class="ul_phone"><?php  echo $theUser->phone ?></span>;
                    <small class="text-muted">Email:</small> <span class="ul_email"><?php  echo $theUser->email ?></span>
                </p>
