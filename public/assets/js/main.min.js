...
"undefined"!=typeof angular&&app.controller("calendarController",["$scope",function(e){function t(t){for(var a=0;a<e.rows.length;a++)if(e.rows[a].id==t)return!0;return!1}e.sessions=JSON.parse($('input[name="sessions"]').val()),e.rows=[],e.activeDate,e.cartItems=JSON.parse(CART),$("#class-calendar").datepicker({format:"yyyy-mm-dd",startDate:"+1d",weekStart:1,multidate:!1,beforeShowDay:function(a){var r=a.getFullYear()+"-"+("0"+(a.getMonth()+1)).slice(-2)+"-"+("0"+a.getDate()).slice(-2),n=$.grep(e.sessions,function(e){var t=e.date_time;return t=t.split(/\s+/),t[0]==r});if(n.length>0){for(var s in n)t(n[s].id)||e.rows.push({id:n[s].id,date:new Date(n[s].date_time.replace(/\s+/,"T")),price:n[s].price,remaining:n[s].remaining,default_tickets:n[s].default_tickets,selected:e.cartItems[n[s].id],show:!1});return{enabled:!0,classes:"session"}}return{enabled:!1,classes:!1}}}).on("changeDate",function(t){console.log(t),e.activeDate=t.date,e.$apply(),$(".select-box").selectbox()}),e.activeFilter=function(t){if("undefined"!=typeof e.activeDate){var a=t.date;return a.setHours(0,0,0,0),a.toString()==e.activeDate.toString()}}}]);
