...
"undefined"!=typeof angular&&app.controller("DiscoverController",["$scope","$q",function(e){function t(e){$(".class-snippet").addClass("fade-out"),$(".class-snippet").removeClass("active"),$(e).addClass("active"),$(".mb-scroll").mCustomScrollbar("scrollTo",e,{scrollInertia:500,timeout:10})}e.everciseGroups=laracasts.mapResults,e.results=e.everciseGroups.length,e.view=window.innerWidth>=992?"mapview":"gridview",e.sort="id",e.dropwdownStyle={top:0,left:0},e.active={},e.maxDistance=5,e.myMarkers=[],e.markers=[];var i=function(t){var i={id:t.id,name:t.name,directory:t.user.directory,image:t.image,latitude:t.venue.lat,longitude:t.venue.lng,coords:{latitude:t.venue.lat,longitude:t.venue.lng},icon:"/assets/img/icon_default_small_pin.png",price:t.default_price,rating:t.ratings.length,reviews:t.ratings,description:t.description,stars:e.getStars(t.ratings),capacity:t.capacity,sessions:t.futuresessions,distance:t.distance,nextClassDate:t.futuresessions[0].date_time,nextClassDuration:t.futuresessions[0].duration,link:"/classes/"+t.slug,click:function(){e.clicked(this.model)}};return i};e.distanceFilter=function(t){return t.distance<=e.maxDistance||t==e.active?t:void 0},e.toggle=function(t,i){var s=$(t.target).offset(),n=1.5*$(t.target).height();e.dropwdownStyle={top:s.top+n,left:s.left},$(t.target).parent().hasClass("active")&&window.setTimeout(function(){$("#"+i).removeClass("active"),$(t.target).parent().removeClass("active")},1)},e.closeDropdown=function(t){window.setTimeout(function(){$(".tab-pane-sort").removeClass("active"),$("."+t+"-btn").removeClass("active")},1),window.setTimeout(function(){$(".mb-scroll").mCustomScrollbar("scrollTo","#"+e.lastActiveMarker.id,{scrollInertia:500,timeout:10})},100)},e.setDistance=function(t){e.maxDistance=t},e.changeView=function(t){e.view=t},e.isPreviewOpen=!1,e.returnPreview=function(){e.isPreviewOpen=!1},e.map={center:{latitude:50,longitude:-.2},pan:{},options:{streetViewControl:!1,maxZoom:19,minZoom:3},zoom:12},e.clusterStyles=[{textColor:"white",url:"/assets/img/icon_default_small_pin_number.png",height:43,width:33,anchorText:[-14,9]}],e.clusterOptions={gridSize:8,maxZoom:20,zoom:15,styles:e.clusterStyles},e.clusterEvents={click:function(t,i){angular.forEach(i,function(t){e.clicked(t)})}},e.mask=!1,e.initialLoad=e.everciseGroups.length<200?e.everciseGroups.length:200,e.lastActiveMarker="",e.clicked=function(i){e.active=i,e.distanceFilter(i),e.isPreviewOpen=!0,e.preview.id="preview-"+i.id,e.preview.name=i.name,e.preview.image=i.directory+"/search_"+i.image,e.preview.description=i.description,e.preview.nextClassDate=i.sessions[0].date_time,e.preview.nextClassDuration=i.sessions[0].duration,e.preview.capacity=i.capacity,e.preview.link=i.link,e.preview.sessions=i.sessions,e.preview.reviews=i.reviews,e.lastActiveMarker.icon="/assets/img/icon_default_small_pin.png",e.lastActiveMarker=i,i.icon="/assets/img/icon_default_small_pin_grey.png",e.mask=!0,google.maps.event.trigger(e.map,"resize"),e.map.center={latitude:i.latitude,longitude:i.longitude},e.map.zoom=15,e.mask=!0,t("#"+i.id)},e.getStars=function(e){var t=0;if(e.length>0){for(var i=0;i<e.length;i++)t+=e[i].stars;result=Math.round(t/e.length)}else result=0;return result},e.preview={id:1,image:"",description:"",nextClassDate:"",nextClassDuration:"",capacity:"",link:""},e.hasTickets=function(e){return e.remaining>0?!0:!1},$(window).resize(function(){window.innerWidth<992&&e.$apply(function(){e.view="gridview"})}),e.$watch(function(){return e.map.bounds},function(){for(var t=0;t<e.initialLoad;t++)e.myMarkers.push(i(e.everciseGroups[t]));e.markers=e.myMarkers,$("img.lazy").lazyload({container:$(".snippet-body")})},!0)}]);
