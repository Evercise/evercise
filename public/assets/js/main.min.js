...
"undefined"!=typeof angular&&app.controller("DiscoverController",["$scope","$q",function(e){function t(t){$.each(t,function(t,i){e.preview.sessions[t]=new Date(i.date_time)}),console.log(t)}function i(e){$(".class-snippet").addClass("fade-out"),$(".class-snippet").removeClass("active"),$(e).addClass("active"),$(".mb-scroll").mCustomScrollbar("scrollTo",e,{scrollInertia:500,timeout:10})}e.everciseGroups=laracasts.mapResults,e.results=e.everciseGroups.length,e.view="mapview",e.sort="id",e.dropwdownStyle={top:0,left:0},e.active={},e.distance=10,e.$watch(function(){return e.map.bounds},function(){for(var t=0;t<e.initialLoad;t++)e.myMarkers.push(n(e.everciseGroups[t]));e.markers=e.myMarkers},!0);var n=function(t){var i={id:t.id,name:t.name,directory:t.user.directory,image:t.image,latitude:t.venue.lat,longitude:t.venue.lng,coords:{latitude:t.venue.lat,longitude:t.venue.lng},icon:"/assets/img/icon_default_small_pin.png",price:t.default_price,rating:t.ratings.length,reviews:t.ratings,description:t.description,stars:e.getStars(t.ratings),capacity:t.capacity,sessions:t.futuresessions,distance:t.distance,nextClassDate:new Date(t.futuresessions[0].date_time.replace(/-/g,"/")),nextClassDuration:t.futuresessions[0].duration,link:"/class/"+t.id,click:function(){e.clicked(this.model)}};return i};e.distanceFilter=function(t){return t.distance<=e.distance||t==e.active?t:void 0},e.toggle=function(t,i){var n=$(t.target).offset(),r=1.5*$(t.target).height();e.dropwdownStyle={top:n.top+r,left:n.left},$(t.target).parent().hasClass("active")&&window.setTimeout(function(){$("#"+i).removeClass("active"),$(t.target).parent().removeClass("active")},1)},e.closeDropdown=function(e){window.setTimeout(function(){$(".tab-pane").removeClass("active"),$("."+e+"-btn").removeClass("active")},1)},e.changeView=function(t){e.view=t},e.isPreviewOpen=!1,e.currentCenter={latitude:50,longitude:-.2},e.map={center:e.currentCenter,pan:{},options:{streetViewControl:!1,maxZoom:20,minZoom:3},zoom:12},e.results=e.everciseGroups.length,e.myMarkers=[],e.clusterStyles=[{textColor:"white",url:"/assets/img/icon_default_small_pin_number.png",height:43,width:33,anchorText:[-14,9]},{textColor:"white",url:"/assets/img/icon_default_large_pin_number.png",height:66,width:51,anchorText:[-22,14]}],e.clusterOptions={gridSize:8,maxZoom:20,zoom:15,styles:e.clusterStyles},e.mask=!1,e.initialLoad=e.everciseGroups.length<200?e.everciseGroups.length:200,e.markers=[],e.clusterEvents={click:function(t,i){angular.forEach(i,function(t){e.clicked(t)})}},e.returnPreview=function(){e.isPreviewOpen=!1},e.lastActiveMarker="",e.clicked=function(n){e.map.zoom=15,e.currentCenter={latitude:n.latitude,longitude:n.longitude},e.active=n,e.distanceFilter(n),e.preview.id="preview-"+n.id,e.preview.image=n.directory+"/search_"+n.image,e.preview.description=n.description,e.preview.nextClassDate=new Date(n.sessions[0].date_time.replace(/-/g,"/")),e.preview.nextClassDuration=n.sessions[0].duration,e.preview.capacity=n.capacity,e.preview.link=n.link,e.preview.sessions=n.sessions,t(n.sessions),e.preview.reviews=n.reviews,e.lastActiveMarker.icon="/assets/img/icon_default_small_pin.png",e.lastActiveMarker=n,n.icon="/assets/img/icon_default_large_pin.png",e.mask=!0,e.isPreviewOpen&&(google.maps.event.trigger(e.map,"resize"),e.map.zoom=15,e.map.center=e.currentCenter),i("#"+n.id),e.isPreviewOpen=!0},e.getStars=function(e){var t=0;if(e.length>0){for(var i=0;i<e.length;i++)t+=e[i].stars;result=Math.round(t/e.length)}else result=0;return result},e.preview={id:1,image:"",description:"",nextClassDate:"",nextClassDuration:"",capacity:"",link:""},$(window).resize(function(){e.$apply(function(){google.maps.event.trigger(e.map,"resize"),e.map.center=e.currentCenter})}),e.$watch("isPreviewOpen",function(t){t?$(".map-wrapper").animate({left:"820px"},100,function(){google.maps.event.trigger(e.map,"resize"),e.map.center=e.currentCenter}):$(".map-wrapper").animate({left:"410px"},100,function(){google.maps.event.trigger(e.map,"resize"),e.map.center=e.currentCenter})})}]);
...
function Cart(t){this.cart=t,this.form="",this.addListeners()}Cart.prototype={constructor:Cart,addListeners:function(){$(document).on("submit","#empty-cart",$.proxy(this.submit,this)),$(document).on("submit",".remove-row",$.proxy(this.submit,this)),$(document).on("submit",".add-to-class",$.proxy(this.submit,this)),$(document).on("change",".btn-select",$.proxy(this.changeSelectDropdown,this))},changeSelectDropdown:function(t){$(t.target).val()?$(t.target).css("z-index",1):$(t.target).css("z-index",0)},submit:function(t){t.preventDefault(),t.stopPropagation(),this.form=$(t.target),this.ajaxSubmit()},ajaxSubmit:function(){var t=this;$.ajax(t.form.attr("action"),{type:"post",data:t.form.serialize(),dataType:"json",beforeSend:function(){t.form.find("input[type='submit']").text();t.form.find('input[type="submit"]').hasClass("add-btn")?t.form.find("input[type='submit']").prop("disabled",!0).text('<span id="cart-loading" class="icon icon-loading ml10"></span>'):t.form.find("input[type='submit']").replaceWith('<span id="cart-loading" class="icon icon-loading ml10"></span>')},success:function(i){i.view?t.updateCart(i):i.refresh&&location.reload()},error:function(t,i,n){console.log(t+" - "+i+" - "+n)},complete:function(){t.form.find("input[type=submit]").prop("disabled",!1),t.form.find("input[type=submit]").text(text),$("#cart-loading").remove()}})},updateCart:function(t){$(".dropdown ").addClass("open"),$(".dropdown-cart").replaceWith(t.view)}};
