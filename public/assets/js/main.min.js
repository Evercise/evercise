...
"undefined"!=typeof angular&&app.controller("calendarController",["$scope","$filter",function(e,t){function a(t){for(var a=0;a<e.rows.length;a++)if(e.rows[a].id==t)return!0;return!1}e.sessions=JSON.parse($('input[name="sessions"]').val()),e.rows=[],e.activeDate,e.cartItems=JSON.parse(CART),$("#class-calendar").datepicker({format:"yyyy-mm-dd",startDate:"+0d",weekStart:1,multidate:!1,beforeShowDay:function(n){var r=n.getFullYear()+"-"+("0"+(n.getMonth()+1)).slice(-2)+"-"+("0"+n.getDate()).slice(-2),s=$.grep(e.sessions,function(e){if(e.remaining>0){var t=e.date_time;return t=t.split(/\s+/),t[0]==r}});if(s.length>0){for(var i in s)if(!a(s[i].id)){var o,r=s[i].date_time,l=r.split(/[^0-9]/),c=parseInt(l[0],10),d=parseInt(l[1],10)-1,p=parseInt(l[2],10),f=parseInt(l[3],10),u=parseInt(l[4],10),g=parseInt(l[5],10);o=new Date(c,d,p,f,u,g);var m=t("date")(o,"hh:mm a");s[i].remaining>0&&e.rows.push({id:s[i].id,date:o,time:m,price:s[i].price,remaining:s[i].remaining,default_tickets:s[i].default_tickets,selected:e.cartItems[s[i].id],show:!1})}return{enabled:!0,classes:"session"}}return{enabled:!1,classes:!1}}}).on("changeDate",function(t){e.activeDate=t.date,e.$apply(),$(".select-box").selectbox()}),e.activeFilter=function(t){if("undefined"!=typeof e.activeDate){console.log("let do this");var a=t.date;return console.log(a),a.setHours(0,0,0,0),console.log(a),a.toString()==e.activeDate.toString()}}}]);
