...
"undefined"!=typeof angular&&app.controller("DiscoverController",["$scope","$q",function(e){function i(e){$(".class-snippet").addClass("fade-out"),$(".class-snippet").removeClass("active"),$(e).addClass("active"),$(".mb-scroll").mCustomScrollbar("scrollTo",e,{scrollInertia:500,timeout:10})}e.everciseGroups=laracasts.mapResults,e.view="mapview",e.sort="id",e.changeView=function(i){e.view=i},e.isPreviewOpen=!1,e.map={center:{latitude:50,longitude:-1},pann:{},options:{streetViewControl:!1,maxZoom:20,minZoom:3},zoom:12},e.results=e.everciseGroups.length,e.myMarkers=[],e.clusterStyles=[{textColor:"white",url:"/assets/img/icon_default_small_pin_number.png",height:43,width:33,anchorText:[-14,9]},{textColor:"white",url:"/assets/img/icon_default_large_pin_number.png",height:66,width:51,anchorText:[-22,14]}],e.clusterOptions={gridSize:8,maxZoom:20,zoom:15,styles:e.clusterStyles},e.mask=!1,e.initialLoad=e.everciseGroups.length<200?e.everciseGroups.length:200,e.markers=[];var s=function(i){var s={id:i.id,name:i.name,image:"/profiles/"+i.user.directory+"/"+i.image,latitude:i.venue.lat,longitude:i.venue.lng,icon:"/assets/img/icon_default_small_pin.png",price:i.default_price,rating:i.ratings.length,reviews:i.ratings,description:i.description,stars:e.getStars(i.ratings),capacity:i.capacity,sessions:i.futuresessions,nextClassDate:new Date(i.futuresessions[0].date_time.replace(/-/g,"/")),nextClassDuration:i.futuresessions[0].duration,link:"/class/"+i.id,click:function(){e.clicked(this.model)}};return s};e.clusterEvents={click:function(i,s){angular.forEach(s,function(i){e.clicked(i)})}},e.returnPreview=function(){e.isPreviewOpen=!1},e.lastActiveMarker="",e.clicked=function(s){e.isPreviewOpen=!0,e.preview.id="preview-"+s.id,e.preview.image="url('"+s.image+"')",e.preview.description=s.description,e.preview.nextClassDate=new Date(s.sessions[0].date_time.replace(/-/g,"/")),e.preview.nextClassDuration=s.sessions[0].duration,e.preview.capacity=s.capacity,e.preview.link=s.link,e.preview.sessions=s.sessions,e.preview.reviews=s.reviews,e.lastActiveMarker.icon="/assets/img/icon_default_small_pin.png",e.lastActiveMarker=s,s.icon="/assets/img/icon_default_large_pin.png",e.map.center={latitude:s.latitude,longitude:s.longitude},e.map.zoom=15,e.mask=!0,i("#"+s.id)},e.getStars=function(e){var i=0;if(e.length>0){for(var s=0;s<e.length;s++)i+=e[s].stars;result=Math.round(i/e.length)}else result=0;return result},e.preview={id:1,image:"",description:"",nextClassDate:"",nextClassDuration:"",capacity:"",link:""},e.$watch(function(){return e.map.bounds},function(){for(var i=0;i<e.initialLoad;i++)e.myMarkers.push(s(e.everciseGroups[i]));e.markers=e.myMarkers},!0)}]);
...
function AddSessions(e){this.form=e,this.calendar=this.form.find("#calendar"),this.daysOfWeek="",this.dates=[],this.submitDates=[],this.index="",this.recur="no",this.recurring=6,this.recurringFor=this.form.find("#recurring-days").val(),this.originalDates=[],this.rows=[],this.currentDates=[],this.init()}AddSessions.prototype={constructor:AddSessions,init:function(){this.calendar.datepicker({format:"yyyy-mm-dd",startDate:"+1d",todayHighlight:!0,multidate:!0}).on("changeDate",$.proxy(this.getCurrentDates,this)),this.daysOfWeek=this.calendar.find(".dow"),this.addListener()},addListener:function(){this.daysOfWeek.on("click",$.proxy(this.dayOfWeek,this)),$('input[name="recurring"]').on("change",$.proxy(this.recurringCheck,this)),this.form.on("submit",$.proxy(this.submitForm,this)),this.form.find("#recurring-days").on("change",$.proxy(this.changeRecurringFor,this))},dayOfWeek:function(e){this.index=$(e.target).index(),this.rows=[],this.setdayOfWeeks()},getCurrentDates:function(e){var t=this;this.currentDates=e.dates,this.dates=[],$.each(this.currentDates,function(e,s){t.dates.push({key:s.valueOf(),dates:s})})},setdayOfWeeks:function(){var e=new Date,t=e.getMonth(),s=this;for(e.setDate(1);e.getDay()!==this.index;)e.setDate(e.getDate()+1);for(;e.getMonth()===t;){var a=new Date(e.getFullYear()+"-"+("0"+(e.getMonth()+1)).slice(-2)+"-"+("0"+e.getDate()).slice(-2)),i=$.grep(s.dates,function(e){return e.key==a.valueOf()});0==i.length?s.dates.push({key:a.valueOf(),dates:a}):s.dates=s.dates.filter(function(e){return e.key!==a.valueOf()}),e.setDate(e.getDate()+7)}this.setCalendarDates()},recurringDates:function(){var e=this;$.each(this.currentDates,function(t,s){e.dates.push({key:s.valueOf(),dates:s}),e.originalDates.push(s);var a=Math.ceil(s.getDate()/7),i=s.getDay(),r=s.getMonth()+1;e.setRecurringDays(i,r,a)})},submitForm:function(e){e.preventDefault();var t=this;$.each(this.calendar.datepicker("getDates"),function(e,s){var a=s.getDate(),i=s.getMonth()+1,r=s.getFullYear();t.submitDates.push(r+"-"+i+"-"+a)}),$('input[name="session_array"]').val(this.submitDates),this.ajaxUpload()},ajaxUpload:function(){var e=this;$.ajax(e.form.attr("action"),{type:"post",data:e.form.serialize(),dataType:"json",beforeSend:function(){e.form.find("input[type='submit']").prop("disabled",!0).parent().after('<span id="session-loading" class="icon icon-loading ml10 mt10"></span>')},success:function(t){$("#update-session").html(t.view),$('input[name="recurring"][value="no"]').click(),e.dates=[],e.submitDates=[],e.originalDates=[],e.setCalendarDates(),$("html, body").animate({scrollTop:$("#update-container").offset().top-20},1e3)},error:function(){console.log("error")},complete:function(){e.form.find("input[type='submit']").prop("disabled",!1),$("#session-loading").remove()}})},setRecurringDays:function(e,t,s){for(var a=this,i=0;i<a.recurringFor;i++){var r,n=new Date;for(n.setMonth(t+i),n.setDate(1);n.getDay()!==e;)n.setDate(n.getDate()+1);for(;Math.ceil(n.getDate()/7)!==s;)n.setDate(n.getDate()+7);var r=new Date(n.getFullYear()+"-"+("0"+(n.getMonth()+1)).slice(-2)+"-"+("0"+n.getDate()).slice(-2));a.dates.push({key:r.valueOf(),dates:r})}this.setCalendarDates()},recurringCheck:function(e){"yes"==$(e.target).val()?(this.recur="yes",this.recurringDates()):(this.recur="no",this.resetCalendarDates())},setCalendarDates:function(){var e=[],t={};$.each(this.dates,function(s,a){t[a.key]||(t[a.key]=!0,e.push(a.dates))}),this.calendar.datepicker("setDates",e)},resetCalendarDates:function(){console.log(this.originalDates),this.calendar.datepicker("setDates",this.originalDates)},changeRecurringFor:function(e){this.recurringFor=$(e.target).val(),"yes"==this.recur&&this.calendar.datepicker("setDates",this.currentDates),$('input[name="recurring"][value="'+this.recur+'"]').trigger("change")}};