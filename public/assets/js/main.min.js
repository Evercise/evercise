...
function trapezium(i){this.wrapper=i,this.btns=[],this.width=i.outerWidth(!0),this.margin=parseInt(this.wrapper.find(".btn").css("margin-right")),this.init()}trapezium.prototype={constructor:trapezium,init:function(){var i=this,r=this.width,t=0,n=1,p=10,e=1,a=0,s=0;this.wrapper.find(".btn").each(function(d,o){if(t+=$(o).outerWidth(!0),t>r){a=t-r;var h=i.margin+a/s+"px";i.wrapper.find("#row-"+n).find(".btn").css({"margin-left":h,"margin-right":h}),t=0,n=++n,i.wrapper.append('<div class="col-sm-'+p+" col-sm-offset-"+e+' mb30" id="row-'+n+'"></div>'),r=i.wrapper.find("#row-"+n).outerWidth(),r>t&&(t+=$(o).outerWidth(!0),i.wrapper.find("#row-"+n).append(o),p-=2,e=++e)}else++s,i.wrapper.find("#row-"+n).append(o)})}};
function categorySelect(t){this.elem=t,this.inputBox=this.elem.parent().find("input"),this.selected="",this.addListenners()}categorySelect.prototype={constructor:categorySelect,addListenners:function(){this.elem.find("a").on("click",$.proxy(this.select,this))},select:function(t){t.preventDefault();var e=$(t.target);this.selected=e.text(),this.inputBox.val(this.selected)}};
