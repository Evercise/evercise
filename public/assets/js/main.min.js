...
function Cart(t){this.cart=t,this.form="",this.maxQty="2000",this.addListeners(),$(".checkout").length&&(this.step=0,this.viewPrice=VIEWPRICE,console.log(this.viewPrice),this.checkout())}Cart.prototype={constructor:Cart,addListeners:function(){$(document).on("submit","#empty-cart",$.proxy(this.submit,this)),$(document).on("submit",".remove-row",$.proxy(this.submit,this)),$(document).on("submit",".add-to-class",$.proxy(this.submit,this)),$(document).on("change",".btn-select",$.proxy(this.changeSelectDropdown,this)),$(document).on("click",".toggle-select .switch a",$.proxy(this.switchQty,this)),$(document).on("click","#stripe-button",$.proxy(this.openStripe,this)),$(window).on("popstate",$.proxy(this.closeStripe,this))},changeSelectDropdown:function(t){$(t.target).hasClass("select-box")?$(t.target).closest(".add-to-class").trigger("submit"):$(t.target).val()?($(t.target).css("z-index",1),$(t.target).closest(".add-to-class").trigger("submit")):$(t.target).css("z-index",0)},switchQty:function(t){t.preventDefault(),t.stopPropagation();var e=$(t.target).closest(".toggle-select");this.maxQty=$(e).data("qty");var s=$(e).data("trigger"),i=$(t.target).attr("href").substring(1),a=$(e).find("#toggle-quantity").val();if("plus"==i){if(a<this.maxQty){var o=Math.max(parseInt(a)+1);$(e).find("#toggle-quantity").val(o),$(e).find("#qty").text(o)}}else if(a>1){var o=Math.max(parseInt(a)-1);$(e).find("#toggle-quantity").val(o),$(e).find("#qty").text(o)}s&&$(t.target).closest(".add-to-class").trigger("submit")},submit:function(t){t.preventDefault(),t.stopPropagation(),this.form=$(t.target),this.ajaxSubmit()},ajaxSubmit:function(){var t=this;$.ajax(t.form.attr("action"),{type:"post",data:t.form.serialize(),dataType:"json",beforeSend:function(){t.form.find('input[type="submit"]').hasClass("add-btn")?t.form.find("input[type='submit']").prop("disabled",!0):t.form.find("input[type='submit']").replaceWith('<span id="cart-loading" class="icon icon-loading"></span>'),t.form.find(".switch").addClass("disabled")},success:function(e){e.view?t.updateCart(e):e.refresh?location.reload():1==e.validation_failed&&t.failedValidation(e)},error:function(t,e,s){console.log(t+" - "+e+" - "+s)},complete:function(){t.form.find("input[type=submit]").prop("disabled",!1),$("#cart-loading").remove(),t.form.find(".switch").removeClass("disabled")}})},updateCart:function(t){$(".cart-dropdown:visible").addClass("open"),$(".cart-dropdown.open .dropdown-cart").replaceWith(t.view),$("#cart-items").html(t.items)},failedValidation:function(t){$("body").append('<div class="mt10 alert alert-danger alert-dismissible fixed" >'+t.errors.custom+'<button type="button" class="close" data-dismiss="alert"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button></div>')},checkout:function(){$("#step-2").on("shown.bs.collapse",function(t){console.log(t),$("#step-1").find(".switch").addClass("hidden"),$("#step-1").find(".switch-back").removeClass("hidden"),$(".cart-progress").find("#progress-2").addClass("complete")})},openStripe:function(t){var e=this;handler.open({name:"Evercise",description:"Checkout",amount:e.viewPrice}),t.preventDefault()},closeStripe:function(){handler.close()}};
