...
"undefined"!=typeof angular&&app.controller("DiscoverController",["$scope","$q",function(e){function t(e){console.log(e),$(".class-snippet").addClass("fade-out"),$(".class-snippet").removeClass("active"),$(e).addClass("active"),$(".mb-scroll").mCustomScrollbar("scrollTo",e,{scrollInertia:500,timeout:10})}e.everciseGroups=laracasts.mapResults,e.results=e.everciseGroups.length,e.view=window.innerWidth>=992?"mapview":"gridview",e.sort="id",e.dropwdownStyle={top:0,left:0},e.active={},e.distance=10,e.myMarkers=[],e.markers=[],e.$watch(function(){return e.map.bounds},function(){for(var t=0;t<e.initialLoad;t++)e.myMarkers.push(i(e.everciseGroups[t]));e.markers=e.myMarkers,$("img.lazy").lazyload({container:$(".snippet-body")})},!0);var i=function(t){var i={id:t.id,name:t.name,directory:t.user.directory,image:t.image,latitude:t.venue.lat,longitude:t.venue.lng,coords:{latitude:t.venue.lat,longitude:t.venue.lng},icon:"/assets/img/icon_default_small_pin.png",price:t.default_price,rating:t.ratings.length,reviews:t.ratings,description:t.description,stars:e.getStars(t.ratings),capacity:t.capacity,sessions:t.futuresessions,distance:t.distance,nextClassDate:t.futuresessions[0].date_time,nextClassDuration:t.futuresessions[0].duration,link:"/classes/"+t.slug,click:function(){e.clicked(this.model)}};return i};e.distanceFilter=function(t){return t.distance<=e.distance||t==e.active?t:void 0},e.toggle=function(t,i){var s=$(t.target).offset(),n=1.5*$(t.target).height();e.dropwdownStyle={top:s.top+n,left:s.left},$(t.target).parent().hasClass("active")&&window.setTimeout(function(){$("#"+i).removeClass("active"),$(t.target).parent().removeClass("active")},1)},e.closeDropdown=function(t){window.setTimeout(function(){$(".tab-pane-sort").removeClass("active"),$("."+t+"-btn").removeClass("active")},1),window.setTimeout(function(){$(".mb-scroll").mCustomScrollbar("scrollTo","#"+e.lastActiveMarker.id,{scrollInertia:500,timeout:10})},100)},e.changeView=function(t){e.view=t},e.isPreviewOpen=!1,e.returnPreview=function(){e.isPreviewOpen=!1},e.currentCenter={latitude:50,longitude:-.2},e.map={center:e.currentCenter,pan:{},options:{streetViewControl:!1,maxZoom:19,minZoom:3},zoom:12},e.clusterStyles=[{textColor:"white",url:"/assets/img/icon_default_small_pin_number.png",height:43,width:33,anchorText:[-14,9]}],e.clusterOptions={gridSize:8,maxZoom:20,zoom:15,styles:e.clusterStyles},e.clusterEvents={click:function(t,i){angular.forEach(i,function(t){e.clicked(t)})}},e.mask=!1,e.initialLoad=e.everciseGroups.length<200?e.everciseGroups.length:200,e.lastActiveMarker="",e.clicked=function(i){e.map.zoom=15,e.currentCenter={latitude:i.latitude,longitude:i.longitude},e.active=i,e.distanceFilter(i),e.preview.id="preview-"+i.id,e.preview.image=i.directory+"/search_"+i.image,e.preview.description=i.description,e.preview.nextClassDate=i.sessions[0].date_time,e.preview.nextClassDuration=i.sessions[0].duration,e.preview.capacity=i.capacity,e.preview.link=i.link,e.preview.sessions=i.sessions,e.preview.reviews=i.reviews,e.lastActiveMarker.icon="/assets/img/icon_default_small_pin.png",e.lastActiveMarker=i,i.icon="/assets/img/icon_default_small_pin_grey.png",e.mask=!0,google.maps.event.trigger(e.map,"resize"),e.map.zoom=15,e.map.center=e.currentCenter,t("#"+i.id),e.isPreviewOpen=!0},e.getStars=function(e){var t=0;if(e.length>0){for(var i=0;i<e.length;i++)t+=e[i].stars;result=Math.round(t/e.length)}else result=0;return result},e.preview={id:1,image:"",description:"",nextClassDate:"",nextClassDuration:"",capacity:"",link:""},e.hasTickets=function(e){return e.remaining>0?!0:!1},$(window).resize(function(){window.innerWidth<992&&e.$apply(function(){e.view="gridview"})})}]);
...
function registerUser(a){this.form=a,this.validIcon="glyphicon glyphicon-ok",this.invalidIcon="glyphicon glyphicon-remove",this.validatingIcon="glyphicon glyphicon-refresh",this.displayName={message:"Your username is not valid",validators:{notEmpty:{message:"Your display name is required"},stringLength:{min:5,max:20,message:"Your display name must be more than 5 and less than 20 characters long"},regexp:{regexp:/^[a-zA-Z0-9_]+$/,message:"Your display name can only consist of alphabetical, number and underscore"}}},this.email={validators:{notEmpty:{message:"Your email is required and cannot be empty"},emailAddress:{message:"Your email is not a valid email address"}}},this.first_name={validators:{notEmpty:{message:"Your first name is required and cannot be empty"},stringLength:{min:2,max:15,message:"Your first name must be more than 2 and less than 15 characters long"}}},this.last_name={validators:{notEmpty:{message:"Your surname is required and cannot be empty"},stringLength:{min:2,max:15,message:"Your surname must be more than 2 and less than 15 characters long"}}},this.password={validators:{notEmpty:{message:"Your password is required and cannot be empty"},stringLength:{min:6,max:32,message:"Your Password must be more than 6 and less than 32 characters long"},identical:{field:"confirmed_password",message:"Your passwords do not match"}}},this.confirmed_password={validators:{notEmpty:{message:"Your password is required and cannot be empty"},stringLength:{min:6,max:32,message:"Your Password must be more than 6 and less than 32 characters long"},identical:{field:"password",message:"Your passwords do not match"}}},this.init()}registerUser.prototype={constructor:registerUser,init:function(){this.form.find("input[type='submit']").prop("disabled",!0),this.validation()},validation:function(){var a=this;this.form.bootstrapValidator({message:"This value is not valid",feedbackIcons:{valid:this.validIcon,invalid:this.invalidIcon,validating:this.validatingIcon},fields:{display_name:this.displayName,email:this.email,first_name:this.first_name,last_name:this.last_name,password:this.password,confirmed_password:this.confirmed_password}}).on("success.form.bv",function(e){e.preventDefault(),a.ajaxUpload()})},ajaxUpload:function(){var a=this;$.ajax(a.form.attr("action"),{type:"post",data:a.form.serialize(),dataType:"json",beforeSend:function(){a.form.find("input[type='submit']").prop("disabled",!0).after('<span id="register-loading" class="icon icon-loading ml10"></span>')},success:function(e){1==e.validation_failed?a.failedValidation(e):window.location.href=e.url},error:function(a,e,s){console.log(a+" - "+e+" - "+s)},complete:function(){a.form.find("input[type=submit]").prop("disabled",!1),$("#register-loading").remove()}})},failedValidation:function(a){self=this;var e=a.errors;$.each(e,function(a,e){self.form.find('input[name="'+a+'"]').parent().hasClass("input-group")?(self.form.find('input[name="'+a+'"]').parent().parent().addClass("has-error"),self.form.find('input[name="'+a+'"]').parent().after('<small class="help-block" data-bv-validator="notEmpty" data-bv-for="'+a+'" data-bv-result="INVALID">'+e+"</small>"),self.form.find('input[name="'+a+'"]').parent().after('<i class="form-control-feedback glyphicon glyphicon-remove" data-bv-icon-for="'+a+'"></i>')):(self.form.find('input[name="'+a+'"]').parent().addClass("has-error"),self.form.find('input[name="'+a+'"]').parent().find(".glyphicon-ok").remove(),self.form.find('input[name="'+a+'"]').after('<small class="help-block" data-bv-validator="notEmpty" data-bv-for="'+a+'" data-bv-result="INVALID">'+e+"</small>"),self.form.find('input[name="'+a+'"]').after('<i class="form-control-feedback glyphicon glyphicon-remove" data-bv-icon-for="'+a+'"></i>'))})}};
