...
function Cart(t){this.cart=t,this.form="",this.maxQty="2000",this.addListeners(),$(".checkout").length&&(this.step=0,this.viewPrice=VIEWPRICE,this.checkout())}Cart.prototype={constructor:Cart,addListeners:function(){$(document).on("submit","#empty-cart",$.proxy(this.submit,this)),$(document).on("submit",".remove-row",$.proxy(this.submit,this)),$(document).on("submit",".add-to-class",$.proxy(this.submit,this)),$(document).on("change",".btn-select",$.proxy(this.changeSelectDropdown,this)),$(document).on("click",".toggle-select .switch a",$.proxy(this.switchQty,this)),$(document).on("click","#stripe-button",$.proxy(this.openStripe,this)),$(window).on("popstate",$.proxy(this.closeStripe,this)),$(document).on("change",'select[name="quantity"]',$.proxy(this.selectQty,this))},changeSelectDropdown:function(t){$(t.target).hasClass("select-box")?$(t.target).closest(".add-to-class").trigger("submit"):$(t.target).val()?($(t.target).css("z-index",1),$(t.target).closest(".add-to-class").trigger("submit")):$(t.target).css("z-index",0)},switchQty:function(t){t.preventDefault(),t.stopPropagation();var e=$(t.target).closest(".toggle-select");this.maxQty=$(e).data("qty");var i=$(e).data("trigger"),n=$(t.target).attr("href").substring(1),a=$(e).find("#toggle-quantity").val();if("plus"==n){if(a<this.maxQty){var s=Math.max(parseInt(a)+1);$(e).find("#toggle-quantity").val(s),$(e).find("#qty").text(s)}}else if(a>1){var s=Math.max(parseInt(a)-1);$(e).find("#toggle-quantity").val(s),$(e).find("#qty").text(s)}i&&$(t.target).closest(".add-to-class").trigger("submit")},selectQty:function(t){var e=$(t.target).val();$(t.target).closest(".qty-wrapper").find(".qty-select").val(e)},submit:function(t){t.preventDefault(),t.stopPropagation(),this.form=$(t.target),this.ajaxSubmit()},ajaxSubmit:function(){var t=this;$.ajax(t.form.attr("action"),{type:"post",data:t.form.serialize(),dataType:"json",beforeSend:function(){t.form.find('input[type="submit"]').hasClass("add-btn")?t.form.find("input[type='submit']").prop("disabled",!0):t.form.find("input[type='submit']").replaceWith('<span id="cart-loading" class="icon icon-loading"></span>'),$(".mask").length&&$(".mask").removeClass("hidden"),t.form.find(".switch").addClass("disabled")},success:function(e){e.view?(t.updateCart(e),$(".mask").length&&$(".mask").addClass("hidden")):e.refresh?location.reload():1==e.validation_failed&&(t.failedValidation(e),$(".mask").length&&$(".mask").addClass("hidden"))},error:function(t,e,i){console.log(t+" - "+e+" - "+i)},complete:function(){t.form.find("input[type=submit]").prop("disabled",!1),$("#cart-loading").remove(),t.form.find(".switch").removeClass("disabled")}})},updateCart:function(t){$(".cart-dropdown:visible").addClass("open"),$(".cart-dropdown.open .dropdown-cart").replaceWith(t.view),$(".cart-items").html(t.items)},failedValidation:function(t){$("body").append('<div class="mt10 alert alert-danger alert-dismissible fixed" >'+t.errors.custom+'<button type="button" class="close" data-dismiss="alert"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button></div>')},checkout:function(){this.newUserForm=$(".checkout #new-user-form"),this.loginForm=$(".checkout .login-form"),this.userType="new";var t=this;$("#step-2").on("shown.bs.collapse",function(){$("#step-1").find(".switch").addClass("hidden"),$("#step-1").find(".switch-back").removeClass("hidden"),$(".cart-progress").find("#progress-2").addClass("complete")}),$(document).on("change",":checkbox",function(e){var i=$(e.target).attr("name");$(":checkbox").prop("checked",!1),$('input[name="'+i+'"]').prop("checked",!0),t.userType=i}),$(document).on("keyup",t.loginForm.find('input[name="email"]'),function(e){t.newUserForm.find('input[name="email"]').val($(e.target).val())}),$(document).on("click","#cart-account",function(e){e.preventDefault(),"new"==t.userType?t.newUserForm.trigger("submit"):t.loginForm.trigger("submit")}),$(document).on("submit",t.newUserForm,$.proxy(this.newUser,this))},openStripe:function(t){var e=this;handler.open({name:"Evercise",description:"Checkout",amount:e.viewPrice}),t.preventDefault()},closeStripe:function(){handler.close()},newUser:function(t){t.preventDefault();var e=this;console.log(e.loginForm.find('input[name="email"]').val()),e.newUserForm.find('input[name="email"]').val(e.loginForm.find('input[name="email"]').val()),$.ajax(e.newUserForm.attr("action"),{type:"post",data:e.newUserForm.serialize(),dataType:"json",beforeSend:function(){$(".has-error").removeClass("has-error"),$(".error-append").remove(),e.newUserForm.find("input[type='submit']").prop("disabled",!0)},success:function(t){1==t.validation_failed?e.failedValidation(t):location.reload()},error:function(t,e,i){console.log(t+" - "+e+" - "+i)},complete:function(){e.newUserForm.find("input[type=submit]").prop("disabled",!1)}})},failedValidation:function(t){var e=this,i=t.errors;$.each(i,function(t,i){e.loginForm.find('input[name="'+t+'"]').parent().addClass("has-error").after('<div class="form-control input-lg input-group has-error error-append">'+i+"</div>")})}};
