...
function Calendar(i){this.container=i,this.daysOfWeek="",this.days="",this.index="",this.recurring=6,this.rows=[],this.firstRecurringDay="",this.init()}Calendar.prototype={constructor:Calendar,init:function(){this.container.datepicker({format:"yyyy-mm-dd",startDate:"+1d",todayHighlight:!0,multidate:!0}),this.daysOfWeek=this.container.find(".dow"),this.days=this.container.find(".day"),this.addListener()},addListener:function(){this.daysOfWeek.on("click",$.proxy(this.recurringDay,this))},recurringDay:function(i){this.index=$(i.target).index();for(var t=0;2>t;t++)this.rows=[],this.setRecurringDays(),this.index=$(i.target).index()},setRecurringDays:function(){for(var i=this,t=0;6>t;t++)i.rows.push(i.container.find("tbody .day:eq("+i.index+")")),i.index=i.index+7;i.clickRecurringDays()},clickRecurringDays:function(){var i=this;$.each(this.rows,function(i,t){$(t.selector+":not(.new)").click()}),i.nextMonth()},nextMonth:function(){$("th.next").first().click()}};