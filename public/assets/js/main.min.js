...
app.service("dataService",function(e){delete e.defaults.headers.common["X-Requested-With"],this.getData=function(){return e({method:"Post",url:"",params:"limit=10, sort_by=created:desc",headers:{Authorization:"Token token="}})}});
...
"undefined"!=typeof angular&&app.controller("searchController",["$scope","$q",function(s){s.mapOptions={disableDefaultUI:!0,panControl:!1},s.map={zoom:8,center:{latitude:51,longitude:-1}},s.mapEvents={},s.results=laracasts.results,s.mapResults=s.results.mapResults;var t=[];s.markers=[],s.$watch(function(){return s.map.bounds},function(){for(var a in s.mapResults){var e=s.mapResults[a];for(var n in e)if(e.hasOwnProperty(n)){for(lng in e[n].location)var r=lng,l=e[n].location[lng];t.push({id:a,latitude:l,longitude:r,icon:"/assets/img/icon_default_small_pin.png"})}}s.markers=t}),s.everciseGroups=s.results.results.hits}]);
