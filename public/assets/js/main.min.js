function redirectTo(e){window.location.href=e.url}function editClass(e){$("#"+e.id).html(e.view),$("#submit-"+e.id).hide(),$("#"+e.id).parent().collapse("show"),$("#infoToggle-"+e.id).removeClass("hide"),$(".update-session").on("submit",function(e){e.preventDefault(),new AjaxRequest($(this),updateHubRow)}),$(".add-session").on("submit",function(e){e.preventDefault(),new AjaxRequest($(this),newSessionAdded)}),$(".toggle-switch").exists(function(){$(this).on("click",function(){new ToggleSwitch($(this))})}),$(".date-picker").datepicker({format:"yyyy-mm-dd",startDate:"+1d",autoclose:!0,todayHighlight:!0})}function updateHubRow(e){alert("row "+e.id+" edited")}function newSessionAdded(e){console.log(e)}
...
$.ajaxSetup({headers:{"X-CSRF-Token":TOKEN}}),$(function(){$(".navbar").exists(function(){new Navigation(this,$(".hero-nav-change"),$(".sticky-fixed-nav"))}),$(".masonry").exists(function(){new Masonry(this)}),$("#user-nav-bar").exists(function(){new Profile(this)}),$(".toggle-switch").exists(function(){$(this).on("click",function(){new ToggleSwitch($(this))})}),$(".map_canvas").exists(function(){map=new Map(this)}),$(".mb-scroll").exists(function(){$(this).mCustomScrollbar({scrollSpeed:10,autoHideScrollbar:!0})}),$(".holder").exists(function(){new ImagePlaceholder}),$("#register-form").exists(function(){self=this,new Validation(this)}),$(".class-preview").exists(function(){new PreviewClassBox(this)}),$(".hide-by-class").exists(function(){var s=0;$(".hide-by-class").each(function(){var i=this;$("."+$(this).attr("href").substr(1)).exists(function(){0==s&&($(i).addClass("active"),$(this).removeClass("hide")),$(i).removeClass("disabled"),s++})}),this.click(function(){$(".hide-by-class").removeClass("active"),$(this).addClass("active"),$(".hide-by-class-element").addClass("hide"),$("."+$(this).attr("href").substr(1)).removeClass("hide")})}),$("#login-form").exists(function(){login=this,$(document).on("submit",login,function(s){s.preventDefault(),new AjaxRequest(login,redirectTo)})}),$(".edit-class-inline").exists(function(){$(this).on("submit",function(s){s.preventDefault(),$(this).find(".btn-toggle-down").addClass("loading"),new AjaxRequest($(this),editClass)})}),$(".dropdown-cart").exists(function(){cart=new Cart(this)})});
