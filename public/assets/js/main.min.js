...
function imageCropper(t){this.elem=t,this.modal=this.elem.find(".modal-cropper"),this.modalImage=this.modal.find("#uploaded-image"),this.uploadForm=this.elem.find("#image-upload-form"),this.galleryImage=$(".gallery-option"),this.croppedForm=this.modal.find("#cropped-image"),this.uploadButton=this.modal.find("input[name='file']"),this.imageSelect=this.uploadForm.find("#image-select"),this.removeButton=this.uploadForm.find("#cover-remove"),this.image=this.modal.find(".bootstrap-modal-cropper img"),this.xInput=this.modal.find("input[name='x']"),this.yInput=this.modal.find("input[name='y']"),this.wInput=this.modal.find("input[name='width']"),this.hInput=this.modal.find("input[name='height']"),this.bwInput=this.modal.find("input[name='box_width']"),this.bhInput=this.modal.find("input[name='box_height']"),this.originalData={},this.ratio=2.35,this.x=0,this.y=0,this.w=0,this.h=0,this.bw=0,this.bh=0,this.init()}imageCropper.prototype={constructor:imageCropper,init:function(){this.addListener()},addListener:function(){this.imageSelect.on("click",$.proxy(this.upload,this)),this.uploadButton.on("change",$.proxy(this.getImage,this)),this.modalImage.on("load",$.proxy(this.openModal,this)),this.modal.on("shown.bs.modal",$.proxy(this.crop,this)),this.modal.on("hidden.bs.modal",$.proxy(this.destroyCrop,this)),this.croppedForm.on("submit",$.proxy(this.submitForm,this)),this.galleryImage.on("click",$.proxy(this.clickGalleryOption,this)),this.removeButton.on("click",$.proxy(this.removeCover,this))},upload:function(){this.uploadButton.trigger("click")},getImage:function(t){if(self=this,t.target.files&&t.target.files[0]){var i=new FileReader;i.onload=function(t){self.modalImage.attr("src",t.target.result)},i.readAsDataURL(t.target.files[0])}},openModal:function(){this.modal.modal("show")},closeModal:function(){this.modal.modal("hide")},crop:function(){self=this,this.image=this.modal.find(".bootstrap-modal-cropper img"),this.image.cropper({data:this.originalData,aspectRatio:this.ratio,done:function(t){self.x=t.x,self.y=t.y,self.w=t.width,self.h=t.height}})},destroyCrop:function(){this.image.cropper("destroy"),self.modalImage.attr("src",null),this.uploadButton.val("")},submitForm:function(t){t.preventDefault(),this.bw=this.image.cropper("getImageData").width,this.bh=this.image.cropper("getImageData").height,this.xInput.val(this.x),this.yInput.val(this.y),this.wInput.val(this.w),this.hInput.val(this.h),this.bwInput.val(this.bw),this.bhInput.val(this.bh),this.ajaxUpload()},ajaxUpload:function(){var t=this.croppedForm.attr("action"),i=new FormData(this.croppedForm[0]),o=this;$.ajax(t,{type:"post",data:i,processData:!1,contentType:!1,beforeSend:function(){o.croppedForm.find("input[type='submit']").prop("disabled",!0).after('<span id="cropping-loading" class="icon icon-loading ml10"></span>')},success:function(t){o.croppedForm.find("input[type=submit]").prop("disabled",!1),o.uploadForm.append('<img src="/'+t.file+'" alt="cover photo" class="img-responsive">')},error:function(t,i,o){console.log(t+" - "+i+" - "+o)},complete:function(){$("#cropping-loading").remove(),o.removeButton.removeClass("hidden"),o.closeModal()}})},clickGalleryOption:function(t){this.uploadForm.append($(t.target).clone().removeClass("gallery-option"))},removeCover:function(){console.log(self.uploadForm.find("img")),self.uploadForm.find("img").remove()}};
function createVenue(t){this.form=t,this.nextButton=this.form.find(".next"),this.init()}createVenue.prototype={constuctor:createVenue,init:function(){this.addListener},addListener:function(){this.nextButton.on("click",$.proxy(this.next,this))},next:function(){console.log($(e.target).attr("href"))}};
...
function CoverSelect(t){this.container=t,this.cover=this.container.find("#cover_image"),this.galleryImage=this.container.find(".gallery-option"),this.currentImage="",this.init()}CoverSelect.prototype={constructor:CoverSelect,init:function(){this.addListener()},addListener:function(){this.galleryImage.on("click",$.proxy(this.clickGalleryOption,this))},clickGalleryOption:function(t){this.cover.html($(t.target).clone().removeClass("gallery-option"))}};