...
var app=angular.module("DiscoverApp",["google-maps".ns(),"infinite-scroll"]);app.config(["GoogleMapApiProvider".ns(),function(i){i.configure({v:"3.16",libraries:"weather,geometry,visualization"})}]);
...
app.controller("DiscoverController",["$scope","$q",function(e){e.map={center:{latitude:45,longitude:-73},zoom:12};var r=function(e){var r={idKey:e.id,name:e.name,image:"/profiles/"+e.user.directory+"/"+e.image,latitude:e.venue.lat,longitude:e.venue.lng};return r};e.everciseGroups=JSON.parse(laracasts.mapResults),e.myMarkers=[],e.initialLoad=e.everciseGroups.length<8?e.everciseGroups.length:8,e.markers=[],e.loadMore=function(){var a=e.markers.length-1;if(e.everciseGroups.length<a+16)var s=e.everciseGroups.length-a;else var s=16;for(var o=1;s>o;o++)e.myMarkers.push(r(e.everciseGroups[a+o]))},e.$watch(function(){return e.map.bounds},function(){for(var a=0;a<e.initialLoad;a++)e.myMarkers.push(r(e.everciseGroups[a]));e.markers=e.myMarkers},!0)}]);
