function redirectTo(o){window.location.href=o.url}
var app=angular.module("DiscoverApp",["google-maps".ns(),"infinite-scroll"]);app.config(["GoogleMapApiProvider".ns(),function(n){n.configure({v:"3.16",libraries:"weather,geometry,visualization"})}]),app.filter("truncate",function(){return function(n,r,e){return isNaN(r)&&(r=10),void 0===e&&(e="..."),n.length<=r||n.length-e.length<=r?n:String(n).substring(0,r-e.length)+e}}),app.filter("repeat",function(){return function(n,r){r=parseInt(r);for(var e=0;r>e;e++)n.push(e);return n}});
$.fn.exists=function(t){var s=[].slice.call(arguments,1);return this.length&&t.call(this,s),this};
$.ajaxSetup({headers:{"X-CSRF-Token":TOKEN}}),$(function(){$(".navbar").exists(function(){new Navigation(this,$(".hero-nav-change"),$(".sticky-fixed-nav"))}),$(".masonry").exists(function(){new Masonry(this)}),$("#user-nav-bar").exists(function(){new Profile(this)}),$(".toggle-switch").exists(function(){new ToggleSwitch(this)}),$(".map-wrapper").exists(function(){}),$(".holder").exists(function(){new ImagePlaceholder}),$("#register-form").exists(function(){self=this,new Validation(this)}),$(".class-preview").exists(function(){new PreviewClassBox(this)})});
...
app.controller("DiscoverController",["$scope","$q",function(e){e.map={center:{latitude:45,longitude:-73},zoom:12};var r=function(r){var s={idKey:r.id,icon:"/assets/img/icon_default_small_pin.png",name:r.name,image:"/profiles/"+r.user.directory+"/"+r.image,latitude:r.venue.lat,longitude:r.venue.lng,price:r.default_price,rating:r.ratings.length,stars:e.getStars(r.ratings),click:function(){e.click(s)}};return console.log(s),s};e.markerClass="",e.everciseGroups=JSON.parse(laracasts.mapResults),e.myMarkers=[],e.getStars=function(e){var r=0;if(e.length>0){for(var s=0;s<e.length;s++)r+=e[s].stars;result=Math.round(r/e.length)}else result=0;return result},e.initialLoad=e.everciseGroups.length<200?e.everciseGroups.length:200,e.click=function(){},e.markers=[],e.loadMore=function(){var s=e.markers.length-1;if(e.everciseGroups.length<s+16)var t=e.everciseGroups.length-s;else var t=16;for(var a=1;t>a;a++)e.myMarkers.push(r(e.everciseGroups[s+a]))},e.$watch(function(){return e.map.bounds},function(){for(var s=0;s<e.initialLoad;s++)e.myMarkers.push(r(e.everciseGroups[s])),console.log(e.everciseGroups[s]);e.markers=e.myMarkers},!0)}]);
